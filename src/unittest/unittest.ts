...
export type TestResult = { showResult: () => void; getResult: () => { message: string, failed: boolean } };

/**
 * Defines a class for creating and managing unit tests.
 */
export class UnitTest {
    private _successAmount: number = 0;
    private _failAmount: number = 0;
     ...
     */
    replaceRec (input: unknown, replacements: { [index: string]: string }): unknown {
        if (input === null || input === undefined) {
            return input;
        } else if (Types.isArray(input)) {
            return input.map((elem: unknown) => this.replaceRec(elem, replacements));
     ...
     */
    validateRec (toValidate: unknown, expected: unknown, path: string, exact: boolean = false): void {
        if (toValidate === expected) {
            ...
            toValidate.forEach((item: unknown, index: number) => {
    ...
    expectException (callback: () => void, expectedException: new (...args: unknown[]) => Error, message: string = ''): boolean {
        try {
            ...
            if (isExpectedException) {
                ...
                this.fail(`Expected exception ${expectedException.name}, but got ${(err as Error).constructor.name}. ${message}`);
            }
            return true;
        }
